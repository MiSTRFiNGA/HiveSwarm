...
function grantWeapon(id){
  ...
  if(heldWeapons.length>=5){// replace weakest by damage
    let wi=0;for(let i=1;i<heldWeapons.length;i++)if((heldWeapons[i].damage||0)<(heldWeapons[wi].damage||0))wi=i;
    heldWeapons[wi]=Object.assign(copy(base),{rank:1});WEAPON=heldWeapons[0];return 'swap'}
  heldWeapons.push(Object.assign(copy(base),{rank:1}));return 'new'}
...
for(let i=enemies.length-1;i>=0;i--){let e=enemies[i],vx=player.x-e.x,vy=player.y-e.y,d=Math.hypot(vx,vy)||1;e.x+=vx/d*e.speed*dt;e.y+=vy/d*e.speed*dt;for(let j=0;j<i;j++){let o=enemies[j],sx=e.x-o.x,sy=e.y-o.y,sd=Math.hypot(sx,sy)||.01,gap=(e.r+o.r)*.8;if(sd<gap){let push=(gap-sd)*.5;e.x+=sx/sd*push;e.y+=sy/sd*push;o.x-=sx/sd*push;o.y-=sy/sd*push}}e.hit=Math.max(0,e.hit-dt);if(d<e.r+player.r){if(!player.inv){player.hp-=e.damage;player.inv=.7;shake=8;burst(player.x,player.y,'#ff718a',14);
        // G6: small push-off so a single shambler cannot pin you in place for the whole pack.
        player.x-=vx/d*18;player.y-=vy/d*18;
        if(player.hp<=0){player.hp=0;if(score>(META.bestScore||0)){META.bestScore=score;saveMeta()}state='dead'}}e.x-=vx/d*28;e.y-=vy/d*28}}
for(let i=bullets.length-1;i>=0;i--){let b=bullets[i];b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;let removed=b.life<=0;for(let j=enemies.length-1;j>=0&&!removed;j--){let e=enemies[j],r=e.r+b.r;if((e.x-b.x)**2+(e.y-b.y)**2<r*r){e.hp-=b.damage;e.hit=.1;sfx('hit',.08);burst(b.x,b.y,WEAPON.color,3);b.pierce--;if(e.hp<=0){score+=10;sfx('kill',.18);codexSee('enemy:'+e.type.id);const elite=(e.type.dropXp||0)>=5;if(elite){META.credits++;saveMeta()}pickups.push({x:e.x,y:e.y,value:(e.type.dropXp||1)*EDIT.drops.xp});
              ...
              burst(e.x,e.y,e.color,12);enemies.splice(j,1)}if(b.pierce<0)removed=true}}if(removed)bullets.splice(i,1)}
...
window.__swarmDbg=()=>({state,wave,elapsed,score,level,xp,nextXp,orbsCollected,
  ...
  weapons:heldWeapons.map(w=>({id:w.id,name:w.name,rank:w.rank||1,damage:w.damage})),
  startWeapon:META.startWeapon||'weapon.pulse',
...
function openMeta(){if(document.querySelector('#meta'))return;let box=document.createElement('div');box.id='meta';box.className='overlay';
  ...
  let render=()=>{const owned=Object.keys(META.ownedWeapons||{}).filter(id=>META.ownedWeapons[id]);
    const armory=(EDIT.weapons||[]).map(w=>{const have=!!META.ownedWeapons[w.id];const start=META.startWeapon===w.id;
      const cost=have?0:(w.id==='weapon.needler'?4:w.id==='weapon.arc'?6:3);
    ...
    box.querySelectorAll('[data-arm]').forEach(b=>b.onclick=()=>{const id=b.dataset.arm;const w=weaponById(id);if(!w)return;
      ...
      const cost=id==='weapon.needler'?4:id==='weapon.arc'?6:3;
