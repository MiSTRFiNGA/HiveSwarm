...
function hudLayout(){
  const t=SAFE_TOP;
  const stageY=32+t, hpY=70+t, hpBottom=80+t, hudBottom=104+t;
  const weaponY=hudBottom+22, weaponBarY=weaponY+8, toastY=weaponBarY+36;
  return {stageY,hpY,hpBottom,hudBottom,weaponY,weaponBarY,toastY};
}
...
const PRELOAD_STEMS=['player','shambler','runner','crawler','necro_node','brute','armored_dead','mutant_enforcer','zombie_colossus','praetorian'];
...
const HUD=hudLayout();
ctx.fillStyle='#dce9e7';ctx.font='700 16px system-ui';
const waveLabel=(stageBoss||bossPendingT>0)?'STAGE BOSS':'WAVE '+(stageWave+1)+'/'+WAVES_PER_STAGE;
ctx.fillText('STAGE '+(stage+1)+' · '+curStageCfg().name,18,HUD.stageY);
ctx.textAlign='right';ctx.fillStyle='#9fded2';ctx.fillText(waveLabel,VIEW.w-18,HUD.stageY);ctx.textAlign='left';
ctx.font='14px system-ui';ctx.fillStyle='#a5c3be';
const waveLeft=Math.max(0,waveQuota(stageWave)-waveSpawned)+enemies.length;
ctx.fillText((stageBoss?(stageBoss.bossName||'GUARDIAN')+' '+Math.max(0,Math.ceil(stageBoss.hp))+'/'+stageBoss.maxHp:bossPendingT>0?'GUARDIAN INCOMING':'HOSTILES LEFT '+waveLeft)+'   SCORE '+score+'   HOSTILES '+enemies.length+(shieldCharges?'   🛡'+shieldCharges:''),18,55+SAFE_TOP);
...
function renderEntitiesEditor(){
  ...
  if(expAll)expAll.onclick=async()=>{
    const stems=['player','shambler','runner','crawler','necro_node','brute','armored_dead','mutant_enforcer','zombie_colossus','praetorian'];
