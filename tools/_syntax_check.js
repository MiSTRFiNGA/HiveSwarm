...
function weaponArmoryCost(id){return id==='weapon.seeker'?4:id==='weapon.rocket'?5:id==='weapon.poison'?4:id==='weapon.beam'?5:id==='weapon.chain'?5:id==='weapon.nova'?6:3}
...
function fire(){
  ...
  for(const w of heldWeapons){
    ...
    // projectile kinds: bullet / homing / nova / poison / rocket
...
function killEnemy(e,gibs,killOpts){
  ...
  if(wasBoss){stageBoss=null;score+=250;shake=Math.min(10,shake+6);explode(ex,ey,'#ffe066',110,true);state='debrief';startDebrief()}}
...
// 3 rounds per stage then boss (owner 2026-08-08). stageT still tracks total stage time for HUD.
if(!stageBoss){
  stageT+=dt;roundT+=dt;
...
ctx.fillStyle='#dce9e7';ctx.font='700 16px system-ui';ctx.fillText('STAGE '+(stage+1)+' · '+curStageCfg().name+'  ·  ROUND '+(stageBoss?'BOSS':(stageRound+1)+'/'+ROUNDS_PER_STAGE)+'  ·  WAVE '+wave,18,32+SAFE_TOP);ctx.font='14px system-ui';ctx.fillStyle='#a5c3be';
const roundLeft=stageBoss?0:Math.max(0,Math.ceil(roundDuration()-roundT));
ctx.fillText((stageBoss?'GUARDIAN '+Math.max(0,Math.ceil(stageBoss.hp))+'/'+stageBoss.maxHp:(stageRound>=ROUNDS_PER_STAGE-1?'GUARDIAN IN ':'ROUND END ')+roundLeft+'s')+'   SCORE '+score+'   HOSTILES '+enemies.length+(shieldCharges?'   🛡'+shieldCharges:''),18,55+SAFE_TOP);ctx.fillStyle='#26383a';ctx.fillRect(18,70+SAFE_TOP,180,10);ctx.fillStyle=player.hp>30?'#64e7b5':'#ff718a';ctx.fillRect(18,70+SAFE_TOP,180*player.hp/player.maxHp,10);ctx.fillStyle='#dce9e7';ctx.fillText('HP '+Math.ceil(player.hp)+' / '+player.maxHp,205,80+SAFE_TOP);
...
if((state==='play'||state==='levelup'||state==='stagebreak')&&heldWeapons[0]){ctx.save();ctx.textAlign='center';ctx.fillStyle='#ffe066';ctx.font='700 14px system-ui';ctx.shadowColor='#000';ctx.shadowBlur=4;
...
const TABS=['ENTITIES','PLAYER','WAVES','WORLD','STAGES','AUDIO','DATA','BEASTIARY'];
...
const FIELD_HELP={
  ...
  kind:'Fire mode: bullet, homing, beam, chain, nova, poison.',
...
function renderTab(){
  ...
  else if(tab===5){// AUDIO — global toggle + per-enemy Idle/Attack/Die SFX
