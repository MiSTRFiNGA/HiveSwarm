"""Browser-level regression coverage for the HiVE SWARM Forge scaffold.
...
class ForgeBrowserTests(unittest.TestCase):
    ...
    def test_live_value_persists_and_pack_round_trips(self):
        ...
        self.page.set_input_files('#pkLoad', {
            'name': 'forge-roundtrip.hiveswarmpack', 'mimeType': 'application/json',
            'buffer': json.dumps(exported).encode(),
        })
        self.page.wait_for_timeout(350)
        values = json.loads(self.page.evaluate("localStorage.getItem('hive_swarm_forge_v1')"))
    ...
    def test_sprite_save_revert_and_indexeddb_reload(self):
        ...
        records = self.page.evaluate("""async () => new Promise((resolve, reject) => {
          const req=indexedDB.open('hive_swarm_forge_media_v1');
        ...
        reloaded = self.page.evaluate("""async () => new Promise((resolve, reject) => {
          const req=indexedDB.open('hive_swarm_forge_media_v1'); req.onerror=()=>reject(req.error);
    ...
    def test_visual_level_editor_persists_authored_marks_and_scrubs(self):
        ...
        self.page.evaluate("v => localStorage.setItem('hive_swarm_forge_v1', JSON.stringify(v))", values)
