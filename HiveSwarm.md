...
| | |
|---|---|
| **Version** | `0.6.23` · `sw.js` `CACHE_VERSION = v45` |
...
| **APK (one only)** | `C:\Users\MiSTRFiNGA\Desktop\My Games\_APKs\HiveSwarm-0.6.23.apk`. Older Swarm APKs are in `_APKs\Archive`. |
...
### 2026-08-22 — Grok · v0.6.23 · leftover sprite frames

Runner `walk_s` frame 0 was a headless torso — full-body opposite-contact pose. Colossus `walk_s` frame 1 / SE+SW frame 3 were cropped — restored legs. Colossus E/W idle+walk rebuilt from the front **tank** body (the old E was a different skull-saw giant). Runner E idle no longer has baked "HIVE ZOM" text. Crawler S idle scaled up to fill the cell. SE/SW colossus walk is still the skull-and-saw silhouette.
